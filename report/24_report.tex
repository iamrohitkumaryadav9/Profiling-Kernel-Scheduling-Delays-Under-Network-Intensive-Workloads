...
\begin{tabular}{l r l}
\toprule
\textbf{Experiment} & \textbf{Context Switches (M)} & \textbf{Notes} \\
\midrule
E4 (baseline)       & 33.8 & Default \\
E14 (combined, no bpoll) & 35.9 & RPS+pin+CFS \\
E15 (RPS+bpoll)     & 28.4 & Busy poll active \\
E16 (bpoll only)    & 27.3 & Busy poll active \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{\texttt{SO\_BUSY\_POLL} reduces context switches by 25\%.} This is the only mitigation with a measurable system-level effect (E16: 27.3M vs E4: 33.8M), but it cannot compensate for CPU starvation from \texttt{stress-ng}.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Extended Memcached Profiling:} Perform deeper application-level tail latency measurement under the \texttt{memcslap} workload to correlate directly with kernel runqueue delays.
